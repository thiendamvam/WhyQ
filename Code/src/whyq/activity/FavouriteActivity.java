...
public class FavouriteActivity extends Activity implements get_category_delegate {
...
//		progressBar = (ProgressBar)findViewById(R.id.progressBar);
		...
		final CategoryController catController = new CategoryController(FavouriteActivity.this);
	...
	public void onCompletedGetCategory(final ArrayList<Category> categories) {
		...
		categoriesView.setOnItemClickListener(new android.widget.AdapterView.OnItemClickListener() {
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if(position == 0) {
					myIntent.putExtra("allcategory", "allcategory");
					View boardListView = FavouritesActivityGroup.group.getLocalActivityManager() .startActivity("AllCatelogy", myIntent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP)).getDecorView();
